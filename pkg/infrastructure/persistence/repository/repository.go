...
import (
	...
	"github.com/delgerskhn/gosetup/pkg/infrastructure/persistence/specification"
	"gorm.io/gorm"
)
...
func (r *GormRepository[M, E]) Find(ctx context.Context, specifications ...specification.Specification) ([]E, error) {
	return r.FindWithLimit(ctx, -1, -1, specifications...)
}

func (r *GormRepository[M, E]) Count(ctx context.Context, specifications ...specification.Specification) (i int64, err error) {
	model := new(M)
	err = r.getPreWarmDbForSelect(ctx, specifications...).Model(model).Count(&i).Error
	return
}

func (r *GormRepository[M, E]) getPreWarmDbForSelect(ctx context.Context, specification ...specification.Specification) *gorm.DB {
...
func (r *GormRepository[M, E]) FindWithLimit(ctx context.Context, limit int, offset int, specifications ...specification.Specification) ([]E, error) {
	var models []M

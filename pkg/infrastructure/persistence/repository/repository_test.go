...
func TestGormRepository_Find(t *testing.T) {
	...
	many, err := repository.Find(ctx, specification.GreaterOrEqual("weight", 50))
	if err != nil {
		panic(err)
	}
	if len(many) != 2 {
	...
	many, err = repository.Find(ctx, specification.GreaterOrEqual("weight", 90))
	if err != nil {
		panic(err)
	}
	if len(many) != 2 {
	...
	many, err = repository.Find(ctx, specification.And(
		specification.GreaterOrEqual("weight", 90),
